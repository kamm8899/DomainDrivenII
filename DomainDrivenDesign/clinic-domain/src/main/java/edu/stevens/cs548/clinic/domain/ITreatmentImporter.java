...
/**
 * Use this pattern to import a treatment into the domain model without
 * violating the aggregate
 * pattern. The provider provides an implementation of ITreatmentImporter to an
 * external
 * service, and the external service calls one of the operations to input a
 * treatment into the
 * domain model.
 * 
 * The difficult part is how to handle follow-up treatments. Each of these
 * operations returns
 * a consumer that adds any follow-up treatments to the follow-ups for the new
 * treatment, and
