...
@NamedQueries({
		...
		@NamedQuery(name = "SearchPatientWithTreatmentsByPatientId", query = "select p from Patient p left join fetch p.treatments where p.patientId = :patientId"),
		@NamedQuery(name = "CountPatientByPatientId", query = "select count(p) from Patient p where p.patientId = :patientId"),
		@NamedQuery(name = "SearchAllPatients", query = "select p from Patient p"),
